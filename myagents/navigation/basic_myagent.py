...
class BasicAgent(object):
    ...
    def run_step(self, cw_idx):
        ...
        vehicle_speed = get_speed(self._vehicle) / 3.6 # km/h
        ...
        if hazard_detected:
            control = self.add_emergency_stop(control) # throttle을 0으로, brake를 max로
...
# 앞에 agent의 path를 차단하는 vehicle이 있는지 check 하는 Method
    ...
    def _vehicle_obstacle_detected(self, vehicle_list=None, max_distance=None, up_angle_th=90, low_angle_th=0, lane_offset=0):
        ...
        if not max_distance:
            max_distance = self._base_vehicle_threshold # 기본 vehicle_threshold는 5 meter
        ...
        # object의 회전을 사용하여 forward vector를 계산함
